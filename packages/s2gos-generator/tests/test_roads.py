...
from s2gos_generator.resources.roads import (
    Road,
    _fetch_osm_data,
    _get_road_material,
    _get_road_width,
...
class TestParseRoads:
    """`_parse_roads`: OSM ways -> Road segments in scene coords, clipped to bounds."""
...
class TestFetchOsmData:
    def test_malformed_json_file_returns_none(self, tmp_path):
        ...
        assert _fetch_osm_data(cfg, 0.0, 0.0, 1.0, 1.0) is None
...
class TestRoadSidecar:
    ...
    def test_process_target_roads_writes_grouped_sidecar(
        ...
        monkeypatch.setattr(
            "s2gos_generator.resources.roads._fetch_osm_data",
            lambda *a, **k: canned,
        )

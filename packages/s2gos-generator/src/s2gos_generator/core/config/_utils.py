...
def resolve_asset_path(filename: PathRef, asset_type: str = "asset") -> PathRef:
    ...
    """
    try:
        filename = PathRef.model_validate(filename)
        ...
        raise ValueError(
            f"{asset_type} file '{filename.href}' not found in any search path.\n"
            f"Searched in:\n  - {search_paths_str}\n\n"
            f"To fix this:\n"
            f"  1. Add the directory containing '{filename.href}' to asset_search_paths in s2gos_settings.yaml\n"

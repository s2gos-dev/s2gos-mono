...
class SceneResourceContext:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._exclusion_zones: Optional[list] = None
    ...
    def exclusion_zones(self) -> list:
        """All exclusion zones in scene coordinates (``ResolvedExclusionZone``)."""
        if self._exclusion_zones is None:
            from ..processors.exclusion import resolve_exclusion_zones

            self._exclusion_zones = resolve_exclusion_zones(
                self.config, self.coordinate_system
            )
        return self._exclusion_zones

    def exclusion_zones_for(self, target: str) -> list:
        """Exclusion zones that apply to ``target`` ("vegetation" or "buildings")."""
        return [z for z in self.exclusion_zones if target in z.excludes]

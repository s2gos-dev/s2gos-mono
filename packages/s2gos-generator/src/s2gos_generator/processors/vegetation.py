"""Vegetation placement algorithms: placement, spacing, elevation lookup, and
binary serialization of the vegetation collection."""
...
def _process_spillover_vegetation(
    ...
    Pixels within ``max_distance`` of the primary class are processed using a scipy
    distance transform.
...
def _generate_pixel_vegetation_positions(
    ...
    Uses rejection sampling with spacing constraints, bounded by ``max_attempts`` so
...
def _batch_elevation_lookup(
    ...
    """Batch elevation lookup using scipy's RegularGridInterpolator.

    Interpolates elevation for every position from the DEM grid in a single call.
...
def _apply_spacing_filter_optimized(
    ...
    """Spacing filter using a spatial grid.

...
#        Date: 2026-05-12T10:37:22.519304
...
with DAG(
    dag_id="frascati-generation-config",
    ...
    tasks["frascati-generation-config"] = KubernetesPodOperator(
        task_id="frascati-generation-config",
        image="quay.io/s2gos/s2gos-mono:0.0.5",
        cmds=["python", "/app/run_step.py"],
        arguments=[json.dumps({
            "func_module": "s2gos_apps.processes.frascati",
...
"scene_output_dir": "{{ params.scene_output_dir }}"},
    ...
    tasks["__procodile_final_step__"] = PythonOperator(
        ...
        op_kwargs={
            "upstream_task_id": "frascati-generation-config"
        },
        do_xcom_push=True
    )

    tasks["frascati-generation-config"] >> tasks["__procodile_final_step__"]

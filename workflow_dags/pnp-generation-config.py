...
with DAG(
    ...
    "random_seed": Param(type='integer', title='Random Seed', description='RNG seed, mostly for vegetation'),
    "config_output_dir": Param(title='Config Output Dir', description='Generation configuration output directory.', anyOf=[{'type': 'string'}, {'type': 'object', 'properties': {'path': {'type': 'string', 'title': 'Path'}, 'protocol': {'type': 'string', 'title': 'Protocol', 'nullable': True}, 'storage_options': {'type': 'object', 'title': 'Storage Options', 'additionalProperties': {}}}, 'required': ['path'], 'additionalProperties': False}, {'type': 'string', 'format': 'path'}, {'nullable': True}]),
    "scene_output_dir": Param(title='Scene Output Dir', description='Scene description output directiory.', anyOf=[{'type': 'string'}, {'type': 'object', 'properties': {'path': {'type': 'string', 'title': 'Path'}, 'protocol': {'type': 'string', 'title': 'Protocol', 'nullable': True}, 'storage_options': {'type': 'object', 'title': 'Storage Options', 'additionalProperties': {}}}, 'required': ['path'], 'additionalProperties': False}, {'type': 'string', 'format': 'path'}, {'nullable': True}])
    },
) as dag:
    ...
    tasks["pnp-generation-config"] = KubernetesPodOperator(
        task_id="pnp-generation-config",
        image="quay.io/s2gos/s2gos-mono:0.0.5",
        cmds=["python", "/app/run_step.py"],
        arguments=[json.dumps({
            "func_module": "s2gos_apps.processes.pnp",
...
"scene_output_dir": "{{ params.scene_output_dir }}"},
    ...
    tasks["__procodile_final_step__"] = PythonOperator(
        ...
        op_kwargs={
            "upstream_task_id": "pnp-generation-config"
        },
        do_xcom_push=True
    )

    tasks["pnp-generation-config"] >> tasks["__procodile_final_step__"]

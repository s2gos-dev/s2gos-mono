...
with DAG(
    dag_id="kairouan-simulation-config",
    ...
    tasks["kairouan-simulation-config"] = KubernetesPodOperator(
        task_id="kairouan-simulation-config",
...
"config_output_dir": "{{ params.config_output_dir }}"},
    ...
    tasks["__procodile_final_step__"] = PythonOperator(
        ...
        op_kwargs={
            "upstream_task_id": "kairouan-simulation-config"
        },
        do_xcom_push=True
    )

    tasks["kairouan-simulation-config"] >> tasks["__procodile_final_step__"]

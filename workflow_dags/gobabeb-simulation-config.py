...
#        Date: 2026-05-12T11:18:21.675255
...
with DAG(
    dag_id="gobabeb-simulation-config",
    ...
    "gmt_hour": Param(default=9, type='number', title='Gmt Hour', description='Hour of observation at target in GMT time.'),
...
"config_output_dir": "{{ params.config_output_dir }}"},
    ...
    tasks["__procodile_final_step__"] = PythonOperator(
        ...
        op_kwargs={
            "upstream_task_id": "gobabeb-simulation-config"
        },
        do_xcom_push=True
    )

    tasks["gobabeb-simulation-config"] >> tasks["__procodile_final_step__"]
